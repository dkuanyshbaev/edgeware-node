...
// !!! this is a hack for debug !!!
const PRIMARY_PROBABILITY: (u64, u64) = (4, 8);
    ...
    fn filter(&self, c: &Call) -> bool {
        match self {
            ...
            ProxyType::Governance => matches!(
                c,
                Call::Democracy(..) | Call::Council(..) | Call::Treasury(..) | Call::Utility(..)
            ),
            ProxyType::Staking => {
...
impl<F: FindAuthor<u32>> FindAuthor<H160> for EthereumFindAuthor<F> {
        ...
        I: 'a + IntoIterator<Item = (ConsensusEngineId, &'a [u8])>,
    {
        if let Some(author_index) = F::find_author(digests) {
            ...
            let (a, _b) = Babe::authorities()[author_index as usize].clone();

            return Some(H160::from_slice(&a.as_ref()));
...
impl chainbridge::Config for Runtime {
    ...
    type AdminOrigin = frame_system::EnsureOneOf<
        AccountId,
        pallet_collective::EnsureProportionAtLeast<_2, _3, AccountId, CouncilCollective>,
        frame_system::EnsureRoot<AccountId>,
    >;
